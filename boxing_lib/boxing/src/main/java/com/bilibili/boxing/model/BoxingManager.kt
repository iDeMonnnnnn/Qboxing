...
class BoxingManager private constructor() {
    ...
    fun loadMedia(
        cr: ContentResolver, page: Int,
        id: String, callback: IMediaTaskCallback<BaseMedia>
    ) {
        val task =
            if (boxingConfig.isVideoMode) VideoTask() else ImageTask()
        BoxingExecutor.getInstance()
            .runWorker(Runnable { task.load(cr, page, id, callback) })
    }
    ...
    fun loadAlbum(
        ...
        BoxingExecutor.getInstance()
            .runWorker(Runnable { AlbumTask().start(cr, callback) })
    }

    companion object {
        @Volatile
        private var instance: BoxingManager? = null

        @JvmStatic
        fun getInstance(): BoxingManager {
            return instance ?: synchronized(this) {
                instance ?: BoxingManager().also { instance = it }
            }
        }
    }
}